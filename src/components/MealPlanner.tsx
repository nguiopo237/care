      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-6">
          <div className="pb-4 border-b border-slate-100">
            <h3 className="font-sans font-bold text-lg text-[#0fb3a9]">{t("mealPlan.title")}</h3>
          ...
          {mealPlan ? (
            <div className="space-y-6">
              ...
              <div className="p-4 bg-teal-50 rounded-2xl border border-teal-100 flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-teal-700 block uppercase">{t("mealPlan.dailyCalories")}</span>
                  <span className="text-2xl font-bold font-sans text-[#0fb3a9] mt-1 block">
                    {mealPlan.dailyTargetCalories} <span className="text-xs font-medium">{t("mealPlan.kcalPerDay")}</span>
                  </span>
                </div>
                <div className="p-2.5 bg-[#0fb3a9] text-white rounded-xl">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-[#14cec3] rounded-full" 
              ...
              <button
                ...
                className="w-full py-3 border border-[#0fb3a9] hover:bg-teal-50 text-[#0fb3a9] font-bold text-xs rounded-xl transition-all flex items-center justify-center space-x-1.5 cursor-pointer"
              >
                {isGeneratingPlan ? (
                  <>
                  ...
                  <>
                    <Sparkles className="w-3.5 h-3.5 text-[#14cec3]" />
            ...
            <div className="text-center py-6">
              ...
              <button
                ...
                onClick={onGenerateMealPlan}
                disabled={isGeneratingPlan}
                className="w-full py-3 bg-gradient-to-r from-[#14cec3] to-[#0fb3a9] hover:from-[#0fb3a9] hover:to-[#0d9488] text-white font-bold text-xs rounded-xl shadow-sm transition-all cursor-pointer flex items-center justify-center space-x-2"
              >
                {isGeneratingPlan ? (
                  <>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col justify-between">
          {mealPlan && (
            <div className="space-y-6">
              <div>
                <h4 className="font-sans font-bold text-[#0fb3a9] text-base">{t("mealPlan.detailTitle")}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex space-x-3 items-start">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-teal-600 font-semibold bg-teal-50 px-2 py-0.5 rounded-full mt-2.5 inline-block">
                  ...
                  {/* Dejeuner */}
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex space-x-3 items-start">
                    <div className="p-2.5 rounded-xl bg-teal-50 text-teal-600 text-sm font-semibold shrink-0">{t("mealPlan.noon")}</div>
                    <div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-6">
        <div>
          <h3 className="font-sans font-bold text-lg text-[#0fb3a9] flex items-center space-x-2">
            <Leaf className="w-5 h-5 text-[#14cec3] animate-spin-slow" />
        ...
        <form onSubmit={handleSearchRecipes} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0fb3a9]"
            />
          </div>
          <button
            type="submit"
              ...
              <div key={index} className="bg-slate-50 border border-slate-100 p-5 rounded-2xl flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="bg-teal-100 text-teal-800 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded">
                  ...
                  <h4 className="font-sans font-bold text-[#0fb3a9] text-base mt-2.5">{recipe.title}</h4>
                  
                  <div className="bg-teal-50/70 p-3 rounded-xl border border-teal-100/50 my-3 text-xs text-teal-800">
                    <strong className="block text-[10px] font-extrabold uppercase text-teal-700">{t("mealPlan.recipeBenefits")}</strong>
